<template>
    ...
    >
        <v-container slot="prepend" class="text-h6">
            Contents
        </v-container>
        <v-container>
            <v-list flat dense class="py-0 my-0">
                <v-list-group  v-for="(content, i) in contents" :key="i" :append-icon="content.Children.length > 0 ? 'mdi-chevron-down' : ''" class="pl-5 text-body-2 py-0 my-0 font-weight-regular" >
                    <template #activator>
                        <a v-if="content.Children.length = 0" :href="content.ID"></a>
                        <v-list-item-content>
                            <v-list-item-title>
                                {{ content.Title }} 
                            </v-list-item-title>
                    ...
                    <v-list-item v-for="(child, j) in content.Children" :key="j" class="pl-10 text-body-3 py-0 my-0 font-weight-regular" nuxt :to="child.ID">
                        <v-list-item-content>
                            <v-list-item-title>
                                {{ child.Title }}
                            </v-list-item-title>
    ...
    export default {
        ...
        computed: {
                ...
                // eslint-disable-next-line dot-notation
                    if (Portfolio[project].Link === this.route) {
                        for (const skillindex in Portfolio[project].SkillsUsed) {
                            if (modulesjson[Portfolio[project].SkillsUsed[skillindex]].type !== "Language") {
            ...
            ModuleColors () {
                try {
                    ...
                    const increments = Math.floor((cookie.length - 10) / this.ModuleTags.length)
            ...
            LanguageColors () {
                try {
                    ...
                    const increments = Math.floor(cookie.length / this.LanguageTags.length)
                    ...
                    return this.$vuetify.theme.currentTheme.analogic_complement ? this.$vuetify.theme.currentTheme.analogic_complement : undefined
                }
            }
        ...
        watch: {
            ModuleColors: {
                handler(newValue) {
                    const colorstouse = []
                    const increments = Math.floor(newValue.length / this.ModuleTags.length)
                    for (let index = 0; index < this.ModuleTags.length; index++) {
            ...
            LanguageColors: {
                handler(newValue) {
                    const colorstouse = []
                    const increments = Math.floor(newValue.length / this.LanguageTags.length)
                    for (let index = 0; index < this.LanguageTags.length; index++) {
                        // colorstouse.push(newValue[Math.floor(Math.random() * newValue.length)])
        ...
        methods: {
            observeSections() {
                ...
                const sections = document.querySelectorAll('.text-h6, .text-subtitle-1')
                ...
                for (let index = 0; index < titles.length; index++) {
                    if (titles[index].className.includes("text-h6")) {
                        ...
                        if (titles[index].ID === undefined) {
                            titles[index].setAttribute('id', `${titles[index].innerText}`)
                        }
                    ...
                    else if (titles[index].className.includes("text-subtitle-1")) {
                        content[lastmainindex - 1].Children.push({})
                        content[lastmainindex - 1].Children[lastsubindex].Title = titles[index].innerText
                        if (titles[index].ID === undefined) {
                            titles[index].setAttribute('id', `#${titles[index].innerText}`)
                        }
                        content[lastmainindex - 1].Children[lastsubindex].ID = titles[index].id
                        lastsubindex ++
                    }
                }
                setTimeout(() => {
