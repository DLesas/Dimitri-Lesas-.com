            ...
            updatecolor(color) {
                if (this.count !== 0) {
                    ...
                    .then(res => {
                        ...
                        this.$vuetify.theme.currentTheme.secondary.base = res.colors[0].hex.value
                        this.$vuetify.theme.themes.dark.secondary.base = res.colors[0].hex.value
                        this.$vuetify.theme.themes.light.secondary.base = res.colors[0].hex.value
                        this.$cookies.set("secondarybase", res.colors[0].hex.value, "30D")
                        for (let index = 1; index < 6; index++) {
                            const lighten = this.pSBC((0.1 * index), res.colors[0].hex.value)
                            const darken = this.pSBC((-0.1 * index), res.colors[0].hex.value)
                    ...
                    const analogic = []
                    this.$axios.$get(`https://www.thecolorapi.com/scheme?hex=${color.hex.replace("#", "")}&mode=analogic&count=50`)
                    .then(res => {
                    ...
                    this.$axios.$get(`https://www.thecolorapi.com/scheme?hex=${color.hex.replace("#", "")}&mode=analogic-complement&count=6`)
                    .then(res => {
                        for (color in res.colors) {
